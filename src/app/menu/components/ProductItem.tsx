...
const ProductItem = ({ product, addToCart }: IPopularItem) => {
  const image = useLoadImage(product)
  const total = product.price - (product.price * product.discount) / 100

  return (
    <div className="flex aspect-square h-[410px] w-full flex-col items-center justify-around gap-2 bg-secondary/30 border-4 rounded-lg p-2 text-neutral-500 dark:text-neutral-300 transition-all duration-200 border-transparent hover:-translate-y-2 hover:border-b-red-400">
